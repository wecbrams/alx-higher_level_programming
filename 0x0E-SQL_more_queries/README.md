...
Enter password: INTO cities (state_id, name) VALUES (1, "San Francisco");' | mysql -hlocalhost
guillaume@ubuntu:~/$ echo 'INSERT  -uroot -p hbtn_0d_usa
Enter password: 
...
Import the database dump from ``hbtn_0d_tvshows to` your MySQL server: [downloa](https://s3.amazonaws.com/intranet-projects-files/holbertonschool-higher-level_programming+/274/hbtn_0d_tvshows.sql) (same as `13-count_shows_by_genre.sql`)
...
Write a script that lists all shows without the genre `Comedy` in the database h`btn_0d_tvshows`.
